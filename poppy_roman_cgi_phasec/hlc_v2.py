...
#         wavefront0 = proper.prop_get_wavefront(wavefront)
        ...
        wavefront0 *= HLC.fpm_phasor[0,0]                    # apply amplitude & phase from FPM clear area
        nfpm = HLC.fpm_phasor.shape[0]
        n = 1024
        pupil_diam_pix = 309
#         fpm_sampling_lamdivD = fpm_sampling_lam0divD * fpm_lam0_m / lambda_m   
        fpm_sampling_lamdivD = HLC.fpm_pixelscale_lamD * HLC.fpm_ref_wavelength/inwave2.wavelength.to(u.m).value
        wavefront_fpm = mft2(wavefront0, fpm_sampling_lamdivD, pupil_diam_pix, nfpm, +1)   # MFT to highly-sampled focal plane
        wavefront_fpm *= HLC.fpm_mask * (HLC.fpm_phasor - 1)      # subtract field inside FPM region, add in FPM-multiplied region
...
#         inwave2.wavefront = cp.array(proper.prop_shift_center(wavefront0))
